...
class ScrapperImage:
    '''This layer will interact with APIs,webpages and  databases. This layer is about everything happening at backend'''
    ## Create  Image URl
    def createImageUrl(searchterm):
        searchterm=searchterm.split()   #aplitting data based on space gap and the joing using + as in string concatenation
   ...
   # get Raw HTML
    def scrap_html_data(url,header):
        '''What is a header? This func. will get information of header from app.py
        ...
        responseData = response.read()   #get the respone in the form of html
        html = bs(responseData, 'html.parser') #do the parsing based on various tags
        return html
    ...
    def getimageUrlList(rawHtml):
        ...
        for a in rawHtml.find_all("div", {"class": "rg_meta"}):
            link, imageExtension = json.loads(a.text)["ou"], json.loads(a.text)["ity"] #extracting text from 
            # json using a.text and getting the link and extension of that image
